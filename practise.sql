...
WHERE u.city = 'New York' AND e.status = 'upcoming'
...
SELECT u.full_name, u.email, f.rating, f.comments, e.title
FROM Feedback f
...
SELECT e.event_id, e.title, COUNT(r.registration_id) AS total_registrations, AVG(f.rating) AS avg_feedback
FROM Events e
LEFT JOIN Registrations r ON e.event_id = r.event_id
LEFT JOIN Feedback f ON e.event_id = f.event_id
